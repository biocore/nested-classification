...
class Training: 
    ...
    def __innit__(self, mt, ft, folder): 
        self.metadata = mt
    ...
    def train(self):
        ...
        og_df = pd.read_csv(self.metadata, sep='\t')
        ...
        for node in tree.get_tree().traverse(strategy="preorder"):
            ...
            try:
                ...
                meta, ft = qiime_code.create_tree(meta_df, self.feature_table)
            except:
            ...
            if roc_auc[1] > 0.5: 
                ...
                #path = path+taxid
                count+=1
                estimator.save(self.path+str(taxid)+'.qza')
                #final = time.perf_counter()
